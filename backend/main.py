...
async def generate_challenges():
    ...
    try:
        ...
        # Split questions and clean them up robustly
        raw_questions = [q.strip() for q in content.split('\\n') if '?' in q]
        
        cleaned_questions = []
        for q in raw_questions:
            # Strip leading numbers/punctuation that the model might add anyway
            q = re.sub(r'^\\s*\\d+[\\.\\-]?\\s*', '', q)
            # Strip trailing whitespace/question marks, then add one back for consistency
            q = q.strip().rstrip('?').strip() + '?'
            cleaned_questions.append(q)
            
        return {"questions": cleaned_questions[:3]}
    except Exception as e:
